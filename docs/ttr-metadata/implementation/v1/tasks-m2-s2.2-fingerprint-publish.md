...
- [ ] Stage M2.1 DONE bar checked off; `./gradlew :packages:kotlin:ttr-metadata:test` green.
- [ ] `grep -n "kotest-property" gradle/libs.versions.toml` — present (it is in the `kotest` bundle today; property tests below rely on it).
- [ ] `grep -n "kotlin-metadata" .github/workflows/publish.yml PUBLISHING.md` — plan M1.2 was tasked with the `kotlin-metadata/v*` tag wiring + PUBLISHING.md rows. If **absent**, T2.2.6 step 1 adds it (small, in-scope here — do not blocker); if present, T2.2.6 step 1 is a no-op check.
- [ ] Publisher access: either CI does the publish (tag push → `GITHUB_TOKEN`, nothing needed) or, for a manual dry-run, `gpr.user`/`gpr.token` with `write:packages` in `~/.gradle/gradle.properties` (PUBLISHING.md "Local-developer authentication").

## Tasks

### T2.2.1 · Fingerprint spec skeletons + golden canonical form (TEST-FIRST)

- [ ] New specs in `org.tatrman.ttr.metadata.world`, red: `FingerprintCanonicalFormSpec.kt`, `FingerprintStabilitySpec.kt` (property), `FingerprintSensitivitySpec.kt` (property).
- [ ] `FingerprintCanonicalFormSpec` pins the canonical serialization per contracts §5 with a **golden file** `src/testFixtures/resources/fixtures/golden/acme-worlds-dev.canonical.json` (checked in; goldens live beside the shared fixtures so the conformance harness and TTR-P can reuse them). Cases:
...
- [ ] Property-test harness: an `Arb<WorldDocVariant>` that renders the SAME semantic world to TTR text with shuffled def order (`Arb.shuffle`), randomized whitespace, and injected comments — parse → resolve → fingerprint. And a mutation `Arb` producing exactly-one-semantic-field edits (used by sensitivity).
...
- [ ] Every row ends with a disposition: **FIX-HERE** (goes to T2.2.5) or **TTR-P-AMENDMENT** (exact edit text drafted in the notes file, to be applied per plan §6 when TTR-P lists are next touched).
...
- [ ] Implement every FIX-HERE disposition from T2.2.4 (API renames, field additions, fixture adjustments), updating specs in the same commit.
- [ ] Contracts hygiene per the contracts.md header rule ("any drift that changes a shape needs a changelog entry"): add changelog entries to `../../architecture/contracts.md` for (at minimum) the testFixtures path correction (§8) and any §2/§3 shape drift that survived review; bump nothing else.
- [ ] Re-run the two consumer walk-tables in `notes-api-review.md` and flip their ✗ marks to ✓ (or to a linked TTR-P amendment).
...
- [ ] Sweep: `./gradlew build` green (Gradle domain); `pnpm -r test` untouched/green (TS domain unaffected — sanity only).
...
- [ ] `WorldFingerprint` implements contracts §5 exactly: canonical JSON (sorted keys, qname-sorted arrays, defaults elided, locations/comments excluded) → sha256, spelled `sha256:<hex>`; golden canonical form checked in beside the shared fixtures.
- [ ] Property tests pin insensitivity (order/whitespace/comments/default-elision) and sensitivity (engine properties, hosts, staging flag, manifest entries, post-overlay type edits).
- [ ] `LoadIssue` taxonomy final, sealed, id-free, one spec case per category; `LoadResult` never throws on model errors.
- [ ] `notes-api-review.md` complete: every ttr-metadata-consuming step of s1.3 + s2.2 ticked; all seven seed divergences adjudicated; FIX-HERE items shipped; TTR-P amendments drafted verbatim; contracts changelog updated.
...
- [ ] TTR-P s1.3 pre-flight items R2/R3 pass verbatim (plan M2 DONE bar).
