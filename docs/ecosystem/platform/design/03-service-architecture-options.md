...
**AMENDED 2026-07-09 (review-260708 incorporation; Bora's call) → the lineage organ serves COLUMN-LEVEL lineage in v1** — not just program/run/object grain. Column lineage is *derivable* from the compiled plans/expressions (TTR-P review §4.4 established derivability; the compiler already resolves every column reference), so this is a derivation-and-serving commitment, not an authoring surface. Export connectors map it to OpenLineage's column-lineage facet. **Opens CQ-5** (where derivation runs). Amends C-2's organ list; **C is 🟠 dirty** until CQ-5 settles.
...
- ~~**CQ-5** (column-lineage derivation point)~~ **RESOLVED 2026-07-09 → (a) COMPILER-SIDE:** column lineage is derived during compile and emitted into the bundle manifest (the E-5 lineage section) / compile record — MIT per D-3; reviewable artifact content; delegated/external runs get lineage for free; T6's "runtime verifies, never re-derives" honored. **Pinned with it: v1 column lineage is STATIC** (what the plan reads/writes, per compile); run events attach run context — runtime-observed lineage is not promised. F-6's event rider settles: events *cite* the manifest's lineage section, staying thin. · Rejected: (b) server-side plan parsing (re-derives what the compiler knew).
- ~~**CQ-6** (query-door world verification)~~ **RESOLVED 2026-07-09, pinned in one sentence:** *deployed bundles verify their T6 fingerprint; ad-hoc plans through the query door validate against the metadata server's currently-served resolved world* (Argos + the served world; no new mechanism).

## Convergence status

**🟢 C RE-CONVERGED 2026-07-09 (de-dirty pass, Bora):** CQ-5 = compiler-side static column lineage (manifest gains the E-5 lineage section) · CQ-6 pinned (fingerprint for bundles; served world for ad-hoc plans). K's convergence supplied the multi-repo serving model (project roster = server config; platform world = own repo; composition at the resolver).

**Original convergence (2026-07-08)** — C-1 β-spine+α-leaves (γ-ish sequencing) · C-2 γ new metadata server (+ lineage organ + export connectors; Ariadne organs stolen) · C-3 γ two doors, one hall (Proteus dissolves) · C-4 α Charon comes home · C-5-i (c) pluggable validator SPI (CQ-3 ratified). **Riders:** C-5-ii/iii (whois-descendant shape, ingress identity) → **H**; CQ-1 (kantheon transition sequencing) + grey-zone placement sweep (kallimachos, pinakes, report-renderer, metis) → **D**; CQ-2 (run-store ownership) → **F**; CQ-4 → spine-extraction work item. The service map for the platform v1: **metadata server (new, on ttr-metadata) · program door (new, F-proper) · query door (slimmed Theseus) · Argos + validator SPI · Kyklop · workers (arges/brontes/steropes) · Charon · whois-descendant (H) · browser Designer (G).**
