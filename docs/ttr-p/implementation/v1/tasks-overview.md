...
- [ ] [tasks-p3-s3.5-variant-b.md](./tasks-p3-s3.5-variant-b.md) — SQL crunch emit + live A4 variant-B proof (6 tasks): world staging schema, `SqlGraphEmitter` walker, Polars-`right_on` dedup parity, CSV→PG + multi-output export, target-override + variant B, live conform. **SQL join lowering + `coalesce` DONE 2026-07-07** as the fast-follow that seeded this stage; T3.5.1–6 remain.
