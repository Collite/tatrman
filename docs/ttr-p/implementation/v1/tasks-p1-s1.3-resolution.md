...
- [x] Negative fixtures `resolution/negative/` — **25 minimum**, `# expect: TTRP-…` header each. Roster (≥1 fixture per id; pad to 25+ with variants):
  | id | fixture seed |
  |---|---|
...
- [x] Spec skeletons (package `org.tatrman.ttrp.resolve`), red: `TtrpManifestSpec.kt`, `TtrpWorldResolutionSpec.kt`, `TtrpQnameResolutionSpec.kt`, `TtrpErRewriteSpec.kt`, `TtrpSchemaPrecedenceSpec.kt`, `TtrpResolutionNegativeSpec.kt` (table-driven over `negative/`), and in ttrp-cli `TtrpCheckCliSpec.kt`.
...
- [x] Add a TOML library to `gradle/libs.versions.toml` (recommend `org.tomlj:tomlj` — pure-JVM, spec-complete; pin the current release) + `implementation(libs.tomlj)` in ttrp-frontend.
- [x] `org.tatrman.ttrp.project.TtrpManifest`:
  ```kotlin
...
- [x] Wire the dependency per the pre-flight outcome: `implementation(project(":packages:kotlin:ttr-metadata"))` (if in-repo) or `implementation("org.tatrman:ttr-metadata:<version>")`. ALL model/world reading goes through it — ttrp-frontend must not parse `.ttrm` directly.
- [x] `org.tatrman.ttrp.resolve.WorldResolver`:
...
- [x] `ModelIndex` façade over ttr-metadata: qname → db object | er object | binding chain | world object, package-derived kind (D-a sub-1: the qname path itself distinguishes tiers — no kind sigils).
...
- [x] `org.tatrman.ttrp.resolve.NameResolver` over the Stage 1.1 `DottedRef`s. Resolution order for canonical documents: **document imports > full qnames** (C2-d's in-ports tier applies only inside fragments — P6); same-level ambiguity → `TTRP-RES-002`, never first-wins (P2).
- [x] Position typing (each syntactic position checks its kind — D-b/decision 2026-07-03):
...
- [x] Output: `ResolvedDocument` — every `DottedRef` annotated with `ResolvedRef(kind, qname, target)`; unresolved heads → `TTRP-RES-001` ("no <expected-kind> named `<x>` — checked imports [`erp.er.*`] and the world `acme.worlds.dev`"); dangling imports → `TTRP-RES-006`.
...
- [x] `org.tatrman.ttrp.resolve.ErRewriter`, running immediately after NameResolver (T8 sugar stratum — BEFORE any graph work): er entity refs → db table refs via er2db binding; er attribute refs in expressions → db column refs; `on: relation customer_sales` → the modeled join condition as a Stage 1.2 `Expression` (equality over the bound key columns, port-qualified `left.…`/`right.…`).
- [x] **Mandatory provenance:** every rewritten node/expression carries `Provenance(originQname, originName, originLocation)` (e.g. `AMOUNT ← erp.er.sales_txn.amount`). Diagnostics raised on rewritten nodes MUST render the er spelling first: "…`customerType` (bound to `CUST_TYPE`)…" — add a `TtrpDiagnostic.render()` path that consults provenance; spec-case it (the E-d rationale: the analyst wrote `customerType`, never surface `CUST_TYPE` alone).
- [x] Binding misses: er ref whose entity/attribute has no er2db binding under the world's hosted packages → `TTRP-RES-005` "entity `erp.er.customer` has no er2db binding reachable in world `acme.worlds.dev`; bind it in the model or reference the db object directly".
...
- [x] Grammar addition to `TTRP.g4` (small, this stage): program-level `def schema <name> { col: type, … }` statement + inline `schema: { col: type, … }` literal as an `argValue` alt (the `schemaLiteral` placeholder from Stage 1.1 becomes real). Types reuse Stage 1.2's `typeName` production (S23).
- [x] `SchemaResolver` — the three homes, P2-ordered (D-c-δ): **inline > named-in-program > world-declared**; same-level conflict (duplicate program `def schema` with one name; two inlines on one load) → `TTRP-SCH-001` listing both locations. Ad-hoc `load` of a non-modeled storage object with NO schema in any home → `TTRP-SCH-002` "declare a schema: inline, def schema in the program, or on the storage in the world (schema-on-read is banned, T7)". Unknown type spelling → `TTRP-SCH-003` (closed S23 list; message enumerates the vocabulary).
- [x] Replace Stage 1.2's `DeclaredSchemaSource` seam: `ResolvedSchemaSource` feeds the typechecker real column lists — model objects from ttr-metadata, ad-hoc from declared schemas. The hero's expressions must now typecheck with NO hand-fed schemas.
...
- [x] `packages/kotlin/ttrp-cli/build.gradle.kts`: add `application` plugin, `mainClass = "org.tatrman.ttrp.cli.MainKt"`. `Main.kt`: subcommand dispatch by hand (`args[0] == "check"` — no CLI framework yet; the full `ttrp` CLI with build/run/explain/conform lands P3 per S2, and the framework choice belongs there).
- [x] `ttrp check <file>.ttrp`: manifest walk-up from the file's directory → parse → expressions → resolve → er-rewrite → schema+world position checks; print diagnostics one-per-line `FILE:LINE:COL <ID> <message>` + `  ↳ suggested: <alternative>` when present; exit 0 (no ERRORs) / 1 (ERRORs). No graph construction, no emit — front-half only.
- [x] `TtrpCheckCliSpec.kt` (component test, no process spawn — call the same entry function): check(hero.ttrp) → exit 0, zero ERRORs; check(hero_er.ttrp) → exit 0; check(each negative fixture) → exit 1 with the expected id in output.
- [x] Phase-1 DONE sweep: run the full matrix below, fix stragglers, count the negative corpus (`ls resolution/negative/*.ttrp expr/negative/* negative/*.ttrp | wc -l` ≥ 25 resolution + the Stage 1.1/1.2 sets).
...
- [x] `ttrp check` passes `hero.ttrp` AND `hero_er.ttrp` (exit 0, zero ERROR diagnostics).
- [x] ≥25 curated resolution negative fixtures each produce exactly their named diagnostic with suggested alternative.
- [x] Position typing enforced at `target`/`load`/`store`/`schema:`/`uses world`/`relation` positions with kind-naming messages.
- [x] Every er-rewritten ref carries provenance. **Note (review-001 1.3-B):** the "er-spelled diagnostics" line is served by the `ErRewrite.renderErFirst()` *primitive* (tested), NOT by `TtrpDiagnostic.render()` — in Phase 1 typing stays er-named so diagnostics already show er spellings; wiring provenance into `render()` is deferred until a db-spelled diagnostic can arise (Phase 2).
- [x] Schema precedence inline > program > world with same-level conflict = error; S23 type vocabulary enforced.
- [x] `./gradlew build` green across all modules; whole-repo diagnostics catalogue has no id collisions (add a one-liner spec asserting `TtrpDiagnosticId` id-string uniqueness if not already present).

## Blockers

**2026-07-05 · SCOPED (non-fatal) — er-hero join arm + relation→join-condition synthesis under-provisioned by the shared fixture.**
The design-doc er-hero (`06-model-binding-options.md` §"The er-flavored hero variant") joins `customer` ⋈ `sales_txn` on `relation customer_sales`. The SHARED erp-project fixture (ttr-metadata `src/testFixtures/`, contracts §8) binds ONLY `sales_txn` (entity) and `sales_txn.amount` — `customer`, `customer.customerType`, `customer_sales`, and `sales_txn.{region,branch,customer}` are deliberately **unbound**; in particular `customer.customerType` is ttr-metadata's own `RES-005` unbound seed (`ErBindingChainSpec`). Consequences, and what shipped instead:
- **`hero_er.ttrp` adapted** to the BOUND `sales_txn` arm (`load(sales_txn)` → filter/aggregate on `amount`). It still exercises the FULL entity→table + attribute→column rewrite with mandatory provenance (`sales_txn`→`SALES_TXN`, `amount`→`AMOUNT`) and passes `ttrp check` (exit 0). The `customer`/`customer_sales` join arm is omitted (noted in the fixture header).
- **`ErRewriter`'s relation→join-condition Expression synthesis** (the `on: relation` → port-qualified equality tree) is **NOT implemented** — `dbSpelling` is a placeholder string (`"join-condition($name)"`); only endpoint validation (`TTRP-RES-004`) is real and tested. The Expression-tree machinery (reading bound key columns / joinPairs into an equality tree) is absent, not merely unexercised — deferred together with the upstream `customer_sales` joinPair bindings. (Wording corrected per review-001 1.3-A; earlier text said "implemented as far as the model allows".)
- **Not a fork:** per protocol the shared model was NOT modified. **Upstream ask (ttr-metadata):** add er2db bindings for `customer` (entity + `customerType` attr), `customer_sales` (FK/joinPairs), and the remaining `sales_txn` attrs, PLUS a fresh unbound attribute to keep the `RES-005` seed — then restore the join-based er-hero + the join-condition golden here. Mirrored in `tasks-overview.md` §Blockers register.

Everything else in the stage (manifest, world selection, position typing, declared schemas, `ttrp check`, the ≥25 negative corpus incl. `RES-004`/`RES-005`) is delivered and green.
