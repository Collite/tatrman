...
- [x] Every er-rewritten ref carries provenance; er-spelled diagnostics proven by spec.
...
- **`ErRewriter`'s relation→join-condition Expression synthesis** (the `on: relation` → port-qualified equality tree) is implemented as far as the model allows (endpoint validation → `TTRP-RES-004`; provenance stub on a matched relation), but no fixture exercises the *bound-key-column* expansion because `customer_sales` has no bound endpoints/joinPairs. The golden join-condition tree is deferred.
