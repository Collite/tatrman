...
- [x] **4D1 — Manual chapter.** Add a new chapter (e.g. `15-md-model.md`, or slot after
...
- [x] **4D2 — Binding section.** Document `schema binding` for MD: `md2db_cubelet` (wide/long),
...
- [x] **4D3 — Reference + sidebar.** Add the MD keywords/defs and the `md/*` diagnostic codes to
  `14-reference.md`; add the new chapter to `_Sidebar.md` and `Home.md`.

- [x] **4D4 — Worked example.** Extend (or add beside) the `examples/retail/` project with a small
  MD model + binding so the manual's examples compile clean against the shipped tooling.

- [x] **4D5 — CHANGELOG.** Document the additive **3.1** surface in `CHANGELOG.md`: new `md` schema,
...
- [x] **4D7 — Commit.** `Section MD-4D: MD manual chapter + CHANGELOG`.
