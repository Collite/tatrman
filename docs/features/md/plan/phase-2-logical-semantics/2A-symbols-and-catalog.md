...
- [ ] **2A1 — Tests first (red).** In `packages/semantics/src/__tests__/`:
...
- [ ] **2A2 — Wire `@modeler/md-catalog`.** Add `"@modeler/md-catalog": "workspace:*"` to
...
- [ ] **2A3 — Register MD namespaces.** Extend the symbol-table builder (`symbol-table.ts` /
...
- [ ] **2A4 — Catalog preload.** Add a catalog symbol source (mirror `stock-loader.ts`) that exposes
...
- [ ] **2A6 — Commit.** `Section MD-2A: MD symbol namespaces + calc-catalog preload`.
