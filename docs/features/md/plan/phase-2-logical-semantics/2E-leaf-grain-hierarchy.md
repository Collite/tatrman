...
- [x] **2E1 — Tests first (red), table-driven.** Construct small in-memory map graphs and assert:
...
- [x] **2E2 — Lattice functions.** In `md-lattice.ts`: `computeLeaves(graph)`,
...
- [x] **2E3 — Hierarchy inference.** For each consecutive `(lower, upper)` level (leaf→root):
...
- [x] **2E4 — Surface results.** Expose computed leaves/grain order to later consumers (the cubelet
...
- [x] **2E6 — Commit.** `Section MD-2E: leaf/grain lattice + hierarchy inference`.
