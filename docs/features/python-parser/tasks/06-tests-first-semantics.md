...
- [x] **4.1.1 — `tests/test_qname.py`** (← `qname.test.ts`): `segments`, `last`,
      `parent` for single- and multi-segment qnames; root/empty handling.

- [x] **4.1.2 — `tests/test_package_inference.py`** (← `package-inference.test.ts`):
      `<root>/foo/bar/baz.ttr` → `foo.bar`; a root-level file → empty package.

- [x] **4.1.3 — `tests/test_symbol_table.py`** (← `symbol-table.test.ts`):
...
- [x] **4.1.4 — `tests/test_resolver.py`** (← `resolver.test.ts` /
...
- [x] **4.1.5 — `tests/test_stock_loader.py`** (← `StockLoaderSpec` +
...
- [x] **4.1.6 — `tests/test_validator.py`**: the portable-subset codes only —
      e.g. an unresolved reference → `ttr/unresolved-reference`; a duplicate def →
      `ttr/duplicate-definition`. Do **not** assert the TS-only validators
      (file-ordering, `.ttrg` graph, package-decl, duplicate-search-property).
      > **Deviation (verified against the canon):** the task's *cardinality*
      > example is dropped. The canon portable subset (Kotlin `Validator.kt`,
      > which the §5.1 semantics-conformance dump pins byte-for-byte) emits **no**
      > cardinality / target-shape / type-alias / drill_map diagnostics — those
      > validators live only on the TS side and are explicitly out of the portable
      > subset. Asserting a cardinality code would force the Python validator to
      > diverge from Kotlin and break P5 conformance. The suite instead mirrors
      > `ValidatorSpec.kt`: RequiredPropertyMissing, EntityAttributeNotFound,
      > PrimaryKeyColumnNotFound, FuzzyWithoutSearchable, UnresolvedReference
      > (Warning by default), DuplicateDefinition, plus a clean-doc case.

- [x] **4.1.7 — `tests/test_project.py`**: `load_project(tmp_dir)` over a 2-file
...
- [x] **4.1.8 — Confirm red.** `pytest -q tests/` runs; the new semantics suites
