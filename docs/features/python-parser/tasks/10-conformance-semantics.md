...
- Confirm the golden is current: the TS semantics dump
  ...
  > **Note (done):** the committed `out-ts-sem/` was **stale** — it predated the
  > `db → dbo` default-namespace behaviour (8 files showed `db.<kind>.X` instead
  > of `db.dbo.X`) and was missing 14 fixtures (the embedded-sql `41–53` and
  > `54-view-tags`). Regenerated via `dump-sem`; the refresh reflects current
  > TS/Kotlin semantics (and matches the Python output byte-for-byte). The
  > regenerated golden is committed as part of this stage.
...
- [x] **5.1.1 — `tests/conformance/dump_sem.py` (single files).** For each
...
- [x] **5.1.2 — Normalisation (§5.1).** `resolved` = sorted
...
- [x] **5.1.3 — Multi-document subdirectories.** For each **subdirectory** of
...
- [x] **5.1.4 — Extend `test_conformance.py`.** Add resolution cases: diff
...
- [x] **5.1.5 — Prove the gate fails red.** Temporarily break the resolver (e.g.
      skip the auto-import step) and/or the stock copy; confirm `py-sem-vs-ts`
      goes red with a clear diff; revert.

- [x] **5.1.6 — Make it green.** Resolve any drift until `py-sem-vs-ts` is green
...
- [x] **5.1.7 — `out-py-sem/` gitignored** (CI-only). Extend the `py-dump` CI job
