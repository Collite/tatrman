...
- [x] **B.3.1 — Implement `toProtoQName(e, step)` per contracts §2.2.**
      ```kotlin
...
- [x] **B.3.2 — Verify the stock rule.** Run the auto-import parity case: a bare
...
- [x] **B.3.3 — Verify the nested rule.** Run the nested-ref parity case (relation
      `join` or `er2db_attribute` target referencing `er.entity.artikl.id`): the
      proto `name` must be `"artikl.id"` (parent.child), `package=""`,
      `namespace="entity"`.

- [x] **B.3.4 — Verify the package-drop rule.** A user def in a declared package
...
- [x] **B.3.5 — Green the whole parity spec.**
      ```bash
...
- [x] **B.3.6 — Full suite + lint (legacy still wired).**
      ```bash
...
- [x] **B.3.7 — Commit (do not delete legacy yet).** Revert the TEMP
