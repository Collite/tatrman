...
**Status:** Architecture v1, 2026-06-11. Normative companion to
[`contracts.md`](contracts.md) and [`plan.md`](plan.md). The design rationale and
all decisions live in
[`../embedded-language-blocks.md`](../embedded-language-blocks.md) (referenced
here as **DESIGN**); this document is the build-facing view.
...
| SQL grammars | ANTLR grammars-v4 `tsql`, `postgresql`; DuckDB = postgres-derived | vendored-pinned, lazily patched (DESIGN §12.1/§12.7) |
| SQL parser runtime | `antlr4ng` (same as TTR) | generated lexers/parsers per dialect |
...
3. **Highlight (all hosts).** Run the dialect's SQL *lexer* over
   `TaggedBlockValue.value`; map each token's span back into the `.ttr` file
   (base `valueSource` + uniform `indentWidth`, DESIGN §8); emit LSP semantic
   tokens.
4. **Semantics (desktop only).** Run the dialect's SQL *parser* (error-tolerant)
   over the same `value`; the per-dialect adapter walks the tree into
...
| Module | Adds |
|---|---|
| `@modeler/grammar` | `TAGGED_BLOCK_LITERAL` + `embeddedBlock`; vendored SQL grammars (pinned); tag registry |
| `@modeler/parser` | `TaggedBlockValue` walker; SQL lexer/parser wrappers; source map |
| `@modeler/semantics` | `SqlRefModel` adapters; SQL resolver; identifier folding; diagnostics; `modeler.toml` SQL config |
