...
- [x] **2.1 — Tests first: index the mappings.** In the metadata export test
...
- [x] **2.2 — Tests first: short vs block selection.** Assert: an attribute
...
- [x] **2.3 — Build a mapping lookup.** Implement a helper that groups
...
- [x] **2.4 — Attach entity-level mapping.** In `entityToEntityDef`, when an
...
- [x] **2.5 — Attach attribute / relation mappings for the no-entity-block
...
- [x] **2.6 — Remove the standalone er2db emission.** Delete the
...
- [x] **2.7 — Green + semantic equivalence.** `./gradlew :infra:metadata:test`
...
### Stage 2 DoD
- [x] All entity/attribute/relation mappings are inline; no `er2db_*` standalone
