...
- [ ] **1.1 — Tests first: attribute short form.** Add a Kotest case: an
...
- [ ] **1.2 — Tests first: attribute block form.** An `AttributeDef` whose
  `mapping = MappingPropertyBlock(target = …)` (e.g. `{ target: { column: NAZEV } }`)
  renders the block form. Assert exact string.

- [ ] **1.3 — Tests first: entity-level block with columns map.** An `EntityDef`
...
- [ ] **1.4 — Tests first: relation FK short form.** A `RelationDef` with
...
- [ ] **1.5 — Tests first: round-trip.** Parse `samples/2.1/er.ttr` with
...
- [ ] **1.6 — Implement.** Add a `renderMapping(m: MappingProperty): String`
...
- [ ] **1.7 — Full gate.** `./gradlew :packages:kotlin:ttr-writer:test` green;
...
- [ ] **1.8 — Publish.** Per `PUBLISHING.md`, bump and tag `kotlin-writer/v<x.y.z>`
  (or the bundle tag), let CI publish `org.tatrman:ttr-writer:<new>` to GitHub
  Packages. Record the new version — Stage 3 bumps ai-platform to it. For local
  iteration first, `publishToMavenLocal` per the repo's local-publish recipe.

### Stage 1 DoD
- [ ] Both inline variants render for entity/attribute/relation; round-trip on
