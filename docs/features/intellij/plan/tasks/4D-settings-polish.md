...
- [x] **1.1 — Write `TtrSettingsTest` (IntelliJ fixture/Kotest).**
...
- [x] **2.1 — Implement `TtrSettings` (`PersistentStateComponent`).**
...
- [~] **2.2 — Implement `TtrSettingsConfigurable` (settings UI).** *(Implemented as a Kotlin-UI-DSL `BoundConfigurable` with the three fields, registered via `applicationConfigurable` (parentId `language`). Visual persistence check is a GUI/`runIde` step → 4.E.)*
...
- [x] **2.3 — Make `NodeResolver` and provider read the settings.**
...
- [x] **3.1 — Add the notification group and message bundle.**
...
- [~] **3.2 — Surface the notification with a "Configure…" action.** *(Implemented: `TtrStreamConnectionProvider.start()` catches `NodeNotFoundException` → `TtrNotifications.nodeMissing` (balloon + "Configure…" action opening `TtrSettingsConfigurable`) → clean `CannotStartProcessException`; sub-20 emits `nodeTooOld` warning. The missing-Node throw is unit-tested; the balloon display is a GUI/`runIde` step → 4.E.)*
...
- [x] **4.2 — Add the plugin/file icons.**
...
- [x] `TtrSettingsTest` green; settings persist across IDE restarts. *(State round-trip test green; cross-restart persistence is the `PersistentStateComponent` contract — GUI-confirmable in 4.E.)*
- [~] With Node absent and no override, opening a `.ttr` file shows the actionable notification (not a stack trace); configuring a path and reopening starts the server. *(Logic implemented + unit-tested; balloon display is GUI/`runIde` → 4.E.)*
- [x] `verifyPlugin` passes with complete metadata; plugin icon renders. *(Green 242 → 262; `pluginIcon.svg` packaged. Render is GUI-confirmable in 4.E.)*
- [x] IJ-Q3 resolved and recorded in [`contracts.md` §9](../../design/contracts.md). *(ID kept; IJ-Q4 also resolved — no native FileType.)*
