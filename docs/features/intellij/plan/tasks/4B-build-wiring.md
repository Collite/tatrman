...
- [x] **1.1 — Add an `intellij` recipe to the root `justfile`.**
...
- [x] **1.2 — Smoke-run the produced bundle standalone.**
  ```bash
...
- [x] **2.1 — Add the `copyLspBundle` Gradle `Copy` task.**
...
- [x] **2.2 — Confirm the fail-fast path.**
...
- [x] **3.1 — Build the plugin distribution.**
...
- [x] **3.2 — Inspect the zip contents.**
  ```bash
...
- [x] **3.3 — Document the CI ordering.**
...
- [x] `just intellij`'s bundle step yields a standalone-runnable `server-stdio.mjs` + `stock/` in plugin resources.
- [x] `copyLspBundle` brings in both grammars and fails fast when the server bundle is absent.
- [x] The built `.zip` contains server, stock, and both grammars.
- [x] CI ordering is documented.
