...
- [x] **T1 (test first) — DateaddSpec.** Port `codec/sql/DateaddSpec.kt` (rename imports; reuse
...
- [x] **T2 — Register the built-in date operators.** Add `SqlLibraryOperators.DATEADD` / `DATEDIFF` /
...
- [x] **T3 — Investigate wire SYMBOL round-trip (architecture R2).** The datepart travels as a
...
- [x] **T4 — Datepart coverage.** Confirm `Dateparts.toTimeUnit` covers the T-SQL abbreviations the
...
- [x] **T5 — Unparse round-trip.** Assert a `DATEADD(DAY, 1, col)` translates back to T-SQL-valid SQL
...
- [x] **T6 — Additive-invariant.** Full suite `--rerun-tasks` green at baseline + all P0/P1 specs;
  ktlint clean; no pre-existing test moved.

- [x] **T7 — Wrap.** Tick CEP-P1 in `00-task-management.md`; commit `CEP-P1: DATEADD/DATEDIFF/DATEPART
  validation + wire round-trip`; run the phase-exit `/review`.

## CEP-P1 — findings

- **Wire SYMBOL support ported (R2, confirmed real).** tatrman's `wire/Expressions.kt` had no SYMBOL
  handling; ported the three pieces from the reference: the `SYMBOL_TYPE_TAG` const, the encode case
  (`SqlTypeName.SYMBOL` → `symbol:<EnumSimpleName>` string), and the decode block (rebuild via
  `RexBuilder.makeFlag(TimeUnit.valueOf(...))`; only TimeUnit/TimeUnitRange accepted). The datepart
  now round-trips parse→rel→plan.v1→SQL.
- **Decode entries (emergent, like COLLATE in P0).** `operatorFor` needed `"dateadd"/"datediff"/
  "datepart"/"date_part" -> SqlLibraryOperators.*` (the reference resolves these via the un-ported
  `FunctionCatalog`; explicit entries mirror its intent).
- **Result:** ttr-translator **400 tests green** (P0 baseline 390 + DateaddSpec 10); ktlint clean;
  parser regenerated. DateaddSpec covers canonical units, T-SQL abbreviations (`dd`→`DAY`), the SYMBOL
  round-trip, optimizer survival, MSSQL unparse, the mixed end-to-end, strict unknown-datepart
  failure, the documented `dayofyear`→`DOY` fidelity gap, and DATE_PART.
