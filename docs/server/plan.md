# Tatrman Server — Core v1 Plan (thin, by design)

> **Status:** written 2026-07-10 (design re-open session), per RO-2: the core gets a *thin* plan; the frozen PL plan ([`../platform/plan/plan.md`](../platform/plan/plan.md)) remains the operate-tier reference. This plan sequences the handover's critical path (`../ecosystem/next-steps-260710.md` §5) into gated phases; mechanics it does not restate live in the **extraction inventory** (`kantheon/docs/architecture/fork/extraction-inventory-260710.md`) and the **naming ledger** ([`../platform/design/naming-260710.md`](../platform/design/naming-260710.md)). Companion: [`architecture.md`](./architecture.md).
> **Task lists are generated per phase, at phase start** (the PL corpus's own ⚠ proved the point). **SV-P0's set exists (2026-07-10): [`tasks/00-task-management.md`](./tasks/00-task-management.md), stages S1–S6**, with [`contracts.md`](./contracts.md) as the core contract cut. Later phases get theirs at their start.
...
**Deliverables:** gate 1 — `ttr-metadata` → public packages (drop `0.0.1-LOCAL`); gate 2 — ttr-translator + `ttr-plan-proto` transfer to tatrman under final names (TR-3 completion; FQCNs/wire frozen post-OQ-11); gate 3 — spine service artifacts published from `tatrman-server`. Registry (RO-17): **Maven Central = public** (namespace verified via `tatrman.org`, so it rides the domain transfer to Collite); **GitHub Packages = staging/pre-release only** (anonymous Maven downloads fail there) — gate 1 is thereby the *staging* step; images + the chart → GHCR (OCI); signing key = Collite-held (H-6 trust root).
...
**Deliverables:** **the single chart** (umbrella, all roster services + Designer viewer per RO-9; OIDC/Keycloak-integrated; values contract documented); **`ttr import-schema` arc** (STRAT-8 — db model + the E-R first-cut derivation, the valuable half); **reference Golem (Kotlin+Koog)** productionized against the MCP contract + **the conformance conversation suite** authored (RO-8's executable test — shared with the future Python reference); **docs/DX workstream**: public language docs, quickstart ("stranger with a database → governed answer"), the DFP wiki pattern generalized; Data Academy skeleton with Dolphin when it fits. **Stretch arcs (named, non-gating):** TTR-M → Power BI/OSI projection (+ lossiness ledger) — **the first leg of the PF analysis plane, which joined the Server per RO-23** (design source: `../frontends/design.md` §4); Python+LangGraph Golem (unblocks on OQ-3).
...
| NEW-1 | `ttr-resolver` rewrite shape (undesigned) — time-boxed design pass | SV-P3 pre-flight |
| NEW-2 | conformance conversation suite: format, corpus (DFP-derived, per RO-19 ask ③), pass bar (RO-8) | SV-P4 `/planning` |
