# Tatrman Server — Contracts (core v1)

> **Status:** written 2026-07-10 at SV-P0 planning. The core cut of the contract inventory: what SV-P0/P1 touch, pinned to the letter. The frozen PL contracts doc ([`../platform/plan/contracts.md`](../platform/plan/contracts.md)) remains the reference for the seam schemas (§2–§5 there: snapshot archive, `ttr.lock`, stats, compile record) — those enter force via the **RO-13 core ⚑ review** before the publish gates and are not restated here. Decision ground truth = control room §7 (STRAT/RO entries).
...
| MCP surface (`ttr-meta-mcp`, `ttr-query-mcp`, `ttr-fuzzy-mcp`, later `ttr-grounding-mcp`) + conformance conversation suite | tatrman | Apache-2.0 | RO-8; pinned at SV-P4 `/planning` (NEW-2) |
