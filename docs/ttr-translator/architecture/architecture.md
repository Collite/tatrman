...
|---|---|---|
| `shared/libs/kotlin/query-translator` (38 main + 35 test `.kt`; golden data inline, no test-resource files) | `packages/kotlin/ttr-translator` | whole lib, incl. TransDSL/DFDSL codecs (TR-1); package rename (TR-2). One of the 35 test files (`InMemoryModelHandle.kt`) relocates to `src/testFixtures` (contracts §3) |
...
- **TR-6 · First release `0.8.0`;** semver discipline per `PUBLISHING.md` (breaking API/proto change ⇒ minor pre-1.0). (Set to `0.8.0` at A3 by Bora — the plan map already carried it; supersedes the early `0.1.0` figure. See contracts §7 changelog v3.)
