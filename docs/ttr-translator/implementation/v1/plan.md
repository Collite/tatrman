...
```
A1 toolchain + ttr-plan-proto ─► A2 code move (tests first) ─► A3 publish plumbing + v0.8.0
                                                                    │
...
- **Stage A2 · Code move** ([tasks-a2-code-move.md](./tasks-a2-code-move.md)) — `:packages:kotlin:ttr-translator` scaffold; dependency audit of the source lib (hidden ttr-metadata/ttr-parser leaks); **test sources + resources ported first (red)**; main sources ported; package rewrite `shared.translator` → `org.tatrman.translator` (the lib's actual root package) + source-dir normalization; test-fixtures wiring; ktlint; provenance README.
...
- **Stage B1 · Proto adoption** (`tasks-b1-proto-adoption.md`) — catalog entries; delete the 5 transferred `.proto` files from `shared/proto`; `api(libs.tatrman.ttr.plan.proto)`; `just proto` green via the extracted include path; Kotlin-wide compile+test (same FQCNs — no import churn); Python package/steropes onto the wheel; TS gen verified untouched; duplicate-class guard.
  - *Pre-flight (EXTERNAL GATE):* `kotlin-translator/v0.8.0` resolvable from `https://maven.pkg.github.com/Collite/tatrman`.
...
4. PyPI trusted publishing: `ttr-plan-proto` registered as a project with this repo's `publish-python.yml` as trusted publisher **before** `python-plan/v0.8.0` is pushed (same setup ttr-parser went through).
