...
// - error objects {code,message,data} → typed LspRpcError.
//
// One message per text frame; NO batch (matches ttr-designer-server /lsp). No
...
export class JsonRpcWsClient {
  ...
  private handleFrame(raw: unknown): void {
    ...
    if (msg.error) {
      pending.reject(new LspRpcError(msg.error.code, msg.error.message, msg.error.data));
    } else {
  ...
  /** Send a JSON-RPC notification (no id, no response) — LSP `didOpen`/`didChange`. */
