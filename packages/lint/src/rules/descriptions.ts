...
/** Every localized description in a definition, including nested columns/attributes. */
function* localizedDescriptionsOf(def: Definition): Iterable<LocalizedString> {
  const own = (def as { descriptionLocalized?: LocalizedString }).descriptionLocalized;
  if (own) yield own;
  if (def.kind === 'table' || def.kind === 'view') {
    for (const c of def.columns ?? []) if (c.descriptionLocalized) yield c.descriptionLocalized;
  }
  if (def.kind === 'entity') {
    for (const a of def.attributes ?? []) if (a.descriptionLocalized) yield a.descriptionLocalized;
  }
}
