...
export const CORPUS: ProjectFile[] = [
  { uri: '/proj/pkg/structure.ttr', src: `package pkg
schema db namespace dbo
def table empty { description: "x" }
def column lonely { description: "x" }` },
  { uri: '/proj/pkg/entity.ttr', src: `package pkg
...
def entity withAttrs {
  ...
  { uri: '/proj/pkg/table.ttr', src: `package pkg
schema db namespace dbo
def table orders {
  ...
  { uri: '/proj/pkg/refs.ttr', src: `package pkg
schema er namespace ent
def relation r {
  from: ghost_a
  to: ghost_b
}` },
  { uri: '/proj/pkg/imports.ttr', src: `package pkg
import other.db.dbo.thing
import other.db.dbo.thing
import nowhere.*
schema db namespace dbo
def table t { columns: [def column id { type: int }] }` },
  { uri: '/proj/sub/mismatch.ttr', src: `package totally.wrong
schema db namespace dbo
def table t { columns: [def column id { type: int }] }` },
  { uri: '/proj/sub/missing.ttr', src: `schema db namespace dbo
def table t2 { columns: [def column id { type: int }] }` },
  { uri: '/proj/mygraph.ttrg', src: `graph wrongname {
  objects: []
}` },
  { uri: '/proj/pkg/dupA.ttr', src: `package pkg
schema db namespace dbo
def table dup { columns: [def column id { type: int }] }` },
  { uri: '/proj/pkg/dupB.ttr', src: `package pkg
schema db namespace dbo
def table dup { columns: [def column id { type: int }] }` },
];
