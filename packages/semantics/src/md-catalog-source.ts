import { MD_CALC_CATALOG, MD_CATALOG_VERSION, type CatalogEntry } from '@modeler/md-catalog';
 ...
 * user-overridable in v1; `@modeler/lint` resolves a `def map`'s `calc:` against
 * it and the LSP completes catalog names from it. Re-exported through the
 * semantics barrel so consumers depend only on `@modeler/semantics`.
 */
