    ...
    StringSpec({
        ...
        "a derived-row column not on the target is TTRP-EN-001" {
            val diags =
                RowDerivationChecker.check(
                    target,
                    listOf(row(mapOf("nonesuch" to RowColumnSource.Const("x")))),
                    registry,
                )
        ...
        "a call-fn source whose return type does not fit the column is TTRP-EN-006" {
            ...
            val diags =
                RowDerivationChecker.check(
                    target,
                    listOf(row(mapOf("reversal_of" to RowColumnSource.Call(twr())))),
                    registry,
                )
            ...
            val diags =
                RowDerivationChecker.check(
                    ...
                    listOf(
                        row(
                            mapOf(
                                "reversal_of" to RowColumnSource.Call(wallClock),
                            ),
                        ),
                    ),
                    registry,
