 ...
 *   1. a first-line comment override `-- ttr: dialect=sql` / `# ttr: dialect=pandas|ttrb`
 *      (tolerating leading whitespace only) — WINS over the extension (C3-g-ii);
 *   2. the extension `.ttr.sql` → sql, `.ttr.py` → pandas, `.ttrb` → ttrb (P7);
 *   3. neither → null (the caller emits TTRP-FRG-002).
 */
object DialectMarker {
    private val commentRe = Regex("""^\s*(--|#)\s*ttr:\s*dialect\s*=\s*(sql|pandas|ttrb)\s*$""")
