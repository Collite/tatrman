...
| `metadata/v<x.y.z>[-RELEASE]` | `ttr-metadata` + `ttr-metadata-git` + **`ttr-snapshot`** (lockstep; contracts §1 + PL-P1.S1 — the snapshot archive is the metadata seam's transport, `api` on ttr-metadata). First real tag `metadata/v0.1.0` is cut at **M2.2**, not M1 | `just publish bundle metadata` |
